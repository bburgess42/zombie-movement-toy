...
**Skill expression:** A beginner plays safe — stays Lucid, eats the unguarded civilians first, avoids all threats. An expert manages the sanity curve: lures a threat away from a civilian then circles back to eat, deliberately drops to Slipping for a speed boost through a threat corridor, threads a Feral jump through a gap that Lucid can't reach, and knows when to stop eating and sprint for the exit. The expert's zombie looks reckless but is precisely controlled chaos.

---

## 2. Core Description

**One-sentence summary:** The player navigates platformer levels as a zombie whose sanity constantly drains, eating civilians to restore sanity while avoiding threats that guard those civilians, reaching the level exit before sanity or HP hits zero.
...
2. **Find Civilians** — Player identifies civilian locations. Some civilians are unguarded (safe to eat), others are near threats (risky). Civilians flee when the zombie approaches, requiring the player to chase or corner them.
3. **Eat Civilian** — Player overlaps a civilian to consume it. Sanity restores by `SANITY_PER_EAT` (capped at max). The civilian disappears and a **death scream** alerts all threats within `SCREAM_ALERT_RANGE`, creating a burst of danger. The player must decide: eat now (risk scream alerting guards) or skip this one (risk running out of sanity)?
4. **Avoid/Evade Threats** — Threats patrol near civilians they guard (leash behavior) and chase the player on detection or scream alert. Contact deals HP damage. Player chooses: dodge around (costs time/sanity), take the hit and push through (costs HP), lure the threat away then circle back to eat, or find an alternate route.
...
- Threat encounter: 4-8 times per level (multiple threats, re-encounters during chase, scream responses)
...
| Death scream triggered | All threats within `SCREAM_ALERT_RANGE` (128px) become alerted for `SCREAM_ALERT_DURATION` (3s) | Audio/visual scream indicator. Alerted threats change behavior (rush toward scream location). Player sees threats converging — time to run. |
| Threat guard released | The threat that was guarding this civilian reassigns to the nearest remaining civilian | Threat patrol path visibly changes. Remaining civilians become more heavily guarded. |
...
| Civilian escapes | Civilian flees to a position the zombie can't easily reach (e.g., near a threat cluster) | Civilian is still alive but now in a more dangerous location. Chase cost time (= sanity). Player must decide: keep pursuing or give up. |
...
| `THREAT_PATROL_RANGE` | 128 | 64-256 | Pixels of patrol distance (half left, half right from spawn) | 4 tiles of patrol gives enough movement to feel alive without covering too much ground. Used when threat has no civilian to guard. |
| `THREAT_GUARD_RANGE` | 64 | 32-128 | Pixels — how tightly threat patrols near its guarded civilian | 2 tiles. Tight enough to make eating risky, loose enough to allow approach angles. |
| `THREAT_LEASH_DISTANCE` | 160 | 96-256 | Pixels — max chase distance from guarded civilian | 5 tiles. Threat breaks chase and returns if zombie lures it too far. Enables "lure and eat" strategy. |
| `SCREAM_ALERT_RANGE` | 128 | 64-256 | Pixels — death scream alert radius | 4 tiles (GDD validated). All threats within range become alerted when zombie eats a civilian. Primary tuning knob for eat danger. |
| `SCREAM_ALERT_DURATION` | 3.0 | 1.5-5.0 | Seconds threats investigate after a scream | GDD: 2 rounds ≈ 3s. How long threats pursue the scream location before returning to guard. |

### 4.4 Civilian System

| Parameter | Default Value | Range | Description | Balance Notes |
|-----------|--------------|-------|-------------|---------------|
| `CIVILIAN_COUNT` | 5 | 3-7 | Number of civilians per level | At 5 civilians × 4 sanity each = 20 potential restore. With 3 threats, 2 civilians are unguarded (free) and 3 are guarded (risky). Player eating 4 of 5 (80%) gives comfortable margin. |
...
| `CIVILIAN_SEEK_RANGE` | 128 | 64-192 | Pixels — distance within which civilian drifts toward nearest threat for protection | 4 tiles (GDD). Civilians naturally cluster near their guards. |
| `CIVILIAN_WANDER_SPEED` | 30 | 15-60 | Civilian idle movement speed (px/s) | Slow amble when no zombie threat nearby. Just enough to feel alive. |

### 4.5 Level Structure

| Parameter | Default Value | Range | Description | Balance Notes |
|-----------|--------------|-------|-------------|---------------|
| `THREAT_COUNT` | 3 | 2-5 | Number of threats per level | Start with 3 for the vertical slice. Each guards its nearest civilian. Enough to create obstacles without overwhelming. Scale up in later levels. |
...
| 12 | **All civilians eaten but sanity still draining** | Expected gameplay. No special effect (no starvation mechanic). Threats with no civilian to guard default to standard patrol (no leash). Player must reach exit on remaining sanity. | Won't-handle (by design) |
| 13 | **Threat leash prevents chase** | Threat breaks chase when zombie exceeds `THREAT_LEASH_DISTANCE` (160px) from guarded civilian. Threat returns to guard. This is intentional — enables "lure and eat" strategy. If leash feels too restrictive, increase `THREAT_LEASH_DISTANCE`. | Won't-handle (evaluate in playtest) |
| 14 | **Civilian flees off platform** | Civilians reverse at platform edges (same as threats). Never fall off. Cornered civilians stop at walls — intentional design (cornering is a strategy). | Must-handle |
| 15 | **Death scream alerts threat already chasing** | Alert overrides chase — threat moves to scream location instead. After alert timer expires, threat returns to guard behavior (reassigning if its civilian was the one eaten). | Nice-to-handle |
...
| **Threat guard/leash behavior** | Enabling | Threats patrol near civilians they guard. Eating is inherently dangerous because food and danger are co-located. The leash mechanic means threats return to guard posts after chasing, enabling the "lure and eat" strategy. As civilians die, threats consolidate — later eats are harder. The guard system is what makes the civilian economy work. |
| **Death scream** | Competing | Eating restores sanity (benefit) but triggers a scream that alerts threats (cost). This single mechanic creates the core risk/reward tension: every eat is a calculated decision about whether the sanity gain outweighs the danger response. See CIVILIAN_ECOLOGY_SPEC.md §4 for full analysis. |
| **Squash/stretch + particles (juice system)** | Complementary | Juice feedback communicates loop state: heavy landing shake after a desperate Feral jump to reach a civilian, dust clouds from a panicked reversal away from a scream-alerted threat. The juice layer makes the loop's tension visible and visceral. |
...
| **Level generator pacing (interest curve)** | Enabling | The pacing system (hook → rising → valley → climax → resolution) maps directly to loop pacing: civilians should appear at valley beats (rest/reward), threats cluster at climax beats (peak tension), the exit is the resolution. Civilian and threat placement should follow the level's pacing curve. |

**Emergent Behaviors:**

- **Lure and eat**: Player enters threat detection range to pull a guard away from its civilian, then circles back to eat while the threat is chasing. Death scream fires but the lured threat is already far away. This emerges from leash + detection + scream range interaction.
...
- **Scream chain**: Player eats one civilian, sprints to another, eats that one too. Both screams alert overlapping sets of threats, creating a mob of pursuers. High-risk, high-reward "feeding frenzy" for massive sanity recovery.
...
- **Guard consolidation**: After eating 3 of 5 civilians, all 3 threats guard the remaining 2 civilians. The last civilians become mini-bosses — heavily guarded, extremely risky. The player must decide if the sanity is worth the danger.
...
| **Route choice variance** | Players take different paths on repeated plays | Observation during playtesting — do players explore, or beeline? | All players take the same path every time = civilian/threat placement creates a dominant route. Needs spatial redesign. |
...
- **Civilian AI is new code.** Simple priority-based AI: flee from zombie > seek nearby threat > wander. Edge avoidance at platform edges. Lower complexity than threat AI.
- **Death scream is an event system.** When zombie eats, broadcast a scream event at the civilian's position. All threats within `SCREAM_ALERT_RANGE` subscribe and respond. Simple distance check — no line-of-sight needed.
- **Threat AI state machine is new code.** States: `GUARD_PATROL` ↔ `CHASE` ↔ `RESPOND_TO_SCREAM`. Guard patrol replaces free patrol when a civilian is assigned. Leash distance limits chase range. Reassignment triggers when guarded civilian dies.
...
| Threat entity + guard patrol | Sprint 2 | 2.2 (data structure + guard fields), 2.3 (guard patrol), 2.6 (tile collision) | 1.4 (civilians must exist for guard assignment) |
| Threat chase + scream response | Sprint 2 | 2.4 (chase + scream response behavior) | 2.3 |
...
- **Threat AI complexity creep.** Guard patrol + chase + scream response is the scope for the vertical slice. Resist adding group coordination, flanking, or inter-threat communication until the basic loop is proven fun.
...
**The choice:** Safe path (no civilians, fewer threats) vs. risky path (civilians near threat guards, tighter platforms).

| Question | Answer |
|----------|--------|
| What are the options? | (A) Take the safe path — avoid threats, conserve HP, but no sanity restoration. (B) Take the risky path — enter threat-guarded areas, potentially take damage, but eat civilians to restore sanity. (C) Eat unguarded civilians first, then decide whether to risk guarded ones. |
| What information does the player have? | They can see civilian locations, threat guard positions, and their current sanity/HP. They can estimate which civilians are unguarded (safe) vs. guarded (risky). |
| What are the consequences? | Safe path: HP preserved, sanity continues draining, may not have enough sanity to reach exit. Risky path: may lose HP (threat damage + scream response), but sanity restored. The death scream adds cascading danger beyond just the immediate threat. |
...
| Dominant strategy risk | If all civilians are unguarded, there's no risk. **Mitigation:** With 5 civilians and 3 threats, at least 3 civilians are guarded. The ratio ensures meaningful routing decisions. |
...
| Question | Answer |
|----------|--------|
| What are the options? | (A) Eat now — guaranteed restore, but may waste sanity if near full, and scream alerts nearby threats. (B) Skip — save this civilian for when sanity is lower. Civilian stays alive and continues its behavior. (C) Eat a different, safer civilian instead. |
| What information does the player have? | Current sanity (HUD + vignette), civilian position, nearby threat positions (visible), scream range (learned through play). |
| What are the consequences? | Eat now: up to `SANITY_PER_EAT` (4) wasted if sanity > 8, PLUS death scream fires regardless. Eating at full sanity = pure cost, no benefit. Later: civilian might flee to a worse position, or threats might reposition. Different civilian: may require detour (time = sanity). |
| Is the choice interesting? | **Yes — richer than static pickups.** The death scream adds a cost to EVERY eat, not just ones near threats. Even "safe" eats carry scream risk if a threat patrols into range. The civilian is also moving — waiting has consequences because the civilian's position changes. |
...
| What information does the player have? | Visible portion of the level (can see civilians and threats in view). Current sanity (are they desperate?). How many civilians they've already eaten. Whether the remaining civilians are heavily guarded (consolidation effect). |
| What are the consequences? | Explore: more civilians eaten (higher sanity buffer), but more sanity spent on travel time, more threat encounters, more scream events. Rush: less sanity spent on travel, but no safety buffer. One mistake (take damage, miss a jump) could be fatal. |
| Is the choice interesting? | **Yes — and the civilian system adds a new dimension.** Late-game civilians are harder to eat due to threat consolidation (see CIVILIAN_ECOLOGY_SPEC.md §6). With 5 civilians restoring 4 sanity each and a drain of 0.15/s, eating the 4th civilian nets ~3.25 sanity (4 restore - ~5s × 0.15 drain for approach). But the 5th civilian, now guarded by all 3 threats, might cost 10-15s of dangerous maneuvering = 1.5-2.25 sanity + possible HP damage. The math creates a "when to stop" decision that doesn't exist with static pickups. |
| Dominant strategy risk | If all civilians are equally easy, exploring is always worth it. **Mitigation:** Threat consolidation makes later civilians progressively harder. The natural difficulty curve within each level creates a point where the risk of eating exceeds the benefit. |
...
**Assumptions:** `SANITY_DRAIN_RATE` = 0.15/s, `SANITY_PER_EAT` = 4, `ZOMBIE_MAX_SANITY` = 12, 5 civilians (2 unguarded, 3 guarded), 3 threats, level takes 180s (3 min).
...
────────  ─────                     ──────  ─────
 ...
 18       Eat free civilian #1      12.0    +4 (capped). No scream response — no threats near.
...
110       Approach guarded civ       6.5    Two threats guarding two civilians.
115       Eat guarded civ #4        10.5    SCREAM! Both remaining threats converge.
122       Escape (took 1 hit)        9.4    -1 HP. Two threats chasing for 3s.
160       Skip last civilian         3.7    All 3 threats guard it. Not worth the risk.
...
This model confirms: at 0.15/s drain with 5 civilians, eating 4 (80%) is sufficient but tight. The last civilian is disproportionately risky (all 3 threats guard it) — the player should usually skip it and sprint for the exit. This creates the "when to stop" decision that the civilian ecology enables.
...
| Feb 2026 | 1.1 | Civilian ecology integration | Replaced static brain pickups with living civilians. Added death scream, threat guard/leash, civilian flee AI. See CIVILIAN_ECOLOGY_SPEC.md for full ecology design. |
