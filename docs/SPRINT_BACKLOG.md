...
**Milestone Goal:** Prove the core loop (navigate → eat civilians → manage sanity → survive guards → reach exit) is fun.
...
| **Sprint 2** | Guard AI + game flow | Guards patrol and chase. Level has entrance/exit. Win/lose states work. |
...
**How this advances the milestone:** Delivers goals 1 (sanity drain), 2 (health), 3 (civilians), and partially 6 (lose states). After this sprint, the resource loop exists even without guards.
...
| | Define a civilian entity: `{ x, y, width: 24, height: 24, vx, alive: true, state: 'wander' }`. AI states: WANDER (slow random patrol, 30 px/s), FLEE (run from zombie at 60 px/s when zombie within 96px), SEEK (drift toward nearest guard within 128px for protection). Reverse at platform edges (never fall off). Render as a blue/cyan rectangle with "C" label. Store in `civilians[]` array. Place 4-5 test civilians on the existing Gauntlet map. Add constants to tuning panel: `CIVILIAN_FLEE_SPEED`, `CIVILIAN_FLEE_RANGE`, `CIVILIAN_SEEK_RANGE`, `CIVILIAN_WANDER_SPEED`. | | | | |
| **1.5** | **Implement civilian eat + death scream** | systems-balance | M | 1.1, 1.4 | [ ] |
| | In the game loop, check AABB overlap between zombie and each alive civilian. On overlap: restore sanity by `SANITY_PER_EAT` (default: 4, matching GDD), cap at 12, set `civilian.alive = false`. Trigger death scream: find all guards within `SCREAM_ALERT_RANGE` (128px) of eaten civilian's position, set their `alertTimer = SCREAM_ALERT_DURATION` (3s) and `alertTarget` to civilian's position. Visual feedback: civilian disappears, scream indicator pulse. Add `SANITY_PER_EAT`, `SCREAM_ALERT_RANGE`, `SCREAM_ALERT_DURATION` to tuning panel. | | | | |
...
**How this advances the milestone:** Delivers goals 4 (guard type), 5 (entrance/exit), 6 (win/lose states complete). After this sprint, all core systems exist — the only missing piece is a purpose-built level.

### Tasks

| # | Task | Domain | Size | Dependency | Status |
|---|------|--------|------|-----------|--------|
| **2.1** | **Design decision: guard interaction model** | design-doc | S | None | [ ] |
| | Answer: Can the zombie kill guards, or is it pure avoidance? Options: (A) **Stomp-to-kill** — jump on head kills guard, touching from side/below damages zombie. Classic platformer. (B) **Pure avoidance** — guards cannot be killed, only dodged. Emphasizes movement mastery. (C) **Sanity-dependent** — at Feral tier, the zombie can stomp; at Lucid, it can't. Ties into the power/control tradeoff. Document the decision and rationale in a section at the top of the task file or GDD. This decision shapes ALL of Sprint 2. | | | | |
| **2.2** | **Create guard entity data structure** | code-architecture | S | None | [ ] |
| | Define guard entity: `{ x, y, width: 32, height: 32, vx, hp, state, guardedCivilian, detectionRange, speed, damage, alertTimer, alertTarget }`. Store in `guards[]` array. Render as a red rectangle with "!" label. Add configurable constants: `GUARD_SPEED`, `GUARD_CHASE_SPEED`, `GUARD_DETECTION_RANGE`, `GUARD_DAMAGE`, `GUARD_WATCH_RANGE`, `GUARD_LEASH_DISTANCE`, `GUARD_PATROL_RANGE`. At level start, assign each guard to guard its nearest same-platform civilian. Add to tuning panel. | | | | |
| **2.3** | **Implement guard guard patrol behavior** | code-architecture | M | 2.2, 1.4 | [ ] |
| | State: GUARD_PATROL. Guard patrols within `GUARD_WATCH_RANGE` (64px) of its assigned civilian. If no civilian assigned (all dead), fall back to standard patrol between `patrolLeft` and `patrolRight`. Reverses direction at patrol bounds and platform edges. When guarded civilian is eaten, reassign to nearest remaining civilian (prefer civilians not already guarded by 2+ guards). Render facing direction indicator. | | | | |
| **2.4** | **Implement guard chase + scream response** | code-architecture | M | 2.3 | [ ] |
| | State: CHASE. When zombie is within `GUARD_DETECTION_RANGE` AND within `GUARD_LEASH_DISTANCE` of guarded civilian, switch to CHASE. Move toward zombie at `GUARD_CHASE_SPEED`. If zombie exceeds leash distance from guarded civilian, break chase and return to GUARD_PATROL. State: RESPOND_TO_SCREAM. When death scream fires within `SCREAM_ALERT_RANGE`, set `alertTimer` and move to `alertTarget` (scream location) at chase speed. Attack zombie if encountered en route. After alert timer expires, return to GUARD_PATROL. Scream response overrides chase. | | | | |
| **2.5** | **Implement guard-zombie collision damage** | code-architecture | S | 2.2, 1.2 | [ ] |
| | AABB overlap check between zombie and each guard. On overlap: call `damageZombie(GUARD_DAMAGE)`. Respect invincibility window (zombie flashes, no re-damage during iframes). If using stomp-to-kill (decision 2.1): check if zombie is falling and above guard's midpoint → kill guard instead of damaging zombie. | | | | |
| **2.6** | **Implement guard collision with tiles** | code-architecture | S | 2.2 | [ ] |
| | Guards need basic tile collision: stay on platforms, don't walk through walls. Simpler than zombie collision — guards don't jump. Just horizontal movement + ground check. If no ground ahead, reverse direction (don't walk off ledges). | | | | |
...
| **2.9** | **Place guards + civilians on Gauntlet map** | level-design | S | 2.3, 2.6, 1.4 | [ ] |
| | Add 3 guards and 5 civilians to the existing Gauntlet map. Place 2 civilians far from guards (unguarded/safe). Place 3 civilians near guards (guarded/risky). Verify guard guard assignment picks the correct civilian. One guard on the ground runway, one on a Lucid-tier platform, one guarding a civilian near a vertical challenge. | | | | |
| **2.10** | **Integration test: full loop on Gauntlet** | qa-testing | S | All above | [ ] |
| | Play the Gauntlet map with all systems: sanity drains, civilians restore sanity, guards patrol and chase, exit exists. Can you survive from entrance to exit? Is it fun? Document observations. This is NOT the final level — it's a systems integration test. | | | | |
...
Chase + scream response (2.4) is above the line because it is the mechanical heart of the civilian ecology's risk/reward loop — without it, the death scream fires but guards do not respond, making the scream a visual-only event with no gameplay consequence. Placement (2.9) and integration testing (2.10) can be quick or deferred.

### Sprint 2 Verification

After Sprint 2, open index.html and test:
- [ ] Red guard rectangles patrol near their assigned civilians
- [ ] Guards do not walk through walls or fall off platforms
- [ ] Touching a guard damages the zombie (HP decreases, iframes flash)
- [ ] Guards detect and chase the zombie within leash distance
- [ ] Guards break chase and return to guard when zombie exceeds leash distance
- [ ] Eating a civilian triggers a scream — nearby guards respond and move to scream location
- [ ] After scream alert expires, guards return to guard behavior
- [ ] When a guarded civilian is eaten, the guard reassigns to another civilian
- [ ] (If stomp-to-kill) Jumping on a guard kills it
...
- [ ] Guards + civilians + death scream + sanity drain + exit = the core loop is playable
...
| | Design a purpose-built level for the vertical slice. NOT the Gauntlet — a level designed for the core loop. Requirements: (1) Linear left-to-right flow with entrance and exit. (2) 5 civilians placed to create routing decisions: 2 unguarded (safe, near main path) and 3 guarded (risky, near guards). (3) 3 guards placed to guard civilians and create obstacles. (4) At least one section requiring low sanity to traverse (testing the power/control tradeoff). (5) Completable in 2-5 minutes. Use the generator to create a base layout, then hand-tune. Dimensions: 40-50 tiles wide, 15-20 tiles tall. Sketch the layout with annotations before building. See CIVILIAN_ECOLOGY_SPEC.md for placement guidelines. | | | | |
| **3.2** | **Build Level 1 in code** | level-design | M | 3.1 | [ ] |
| | Encode the designed layout as a tile map string array (like the Gauntlet). Add civilian positions and guard positions as data. Guard-civilian guard assignments happen automatically at level load (each guard guards nearest same-platform civilian). Replace the default map load with Level 1. Keep the Gauntlet available via the "Gauntlet" button. | | | | |
...
| | Play Level 1 repeatedly and tune: (1) Sanity drain rate — is there urgency without desperation? (2) Sanity restore amount — does eating feel meaningful? (3) Civilian placement — does the player have eat-order choices (free vs. guarded)? (4) Death scream range — does eating feel risky near guards? (5) Guard speed, detection range, leash distance — learnable and fair? (6) Guard guard reassignment — does consolidation make later eats harder? (7) Level length — 2-5 minutes? Record all tuned values. | | | | |
| **3.6** | **Guard behavior tuning** | systems-balance | S | 2.3, 2.4 | [ ] |
| | Tune guard constants for the level: patrol speed, chase speed, detection range, patrol range. Guards should be avoidable by a skilled player at any sanity. At low sanity, guards should be harder to dodge (reduced air control, drift) but the player moves faster (can outrun). The interplay between sanity-driven movement and guard avoidance IS the core tension. | | | | |
...
| | Update `docs/CURRENT_STATE.md` with all new systems (sanity drain, health, civilians, guards, level structure, HUD, game states). Update `docs/TODO.md` with completed items and any new items discovered during playtesting. Update `docs/ARCHITECTURE.md` with new sections (guard AI, civilian AI, game states). | | | | |
...
Sprint 2 (Guards + Flow)
─────────────────────────
2.1 (design decision) ──→ shapes 2.5
1.4 (civilians) ──→ 2.2 (guard entity + guard assign) ──→ 2.3 (guard patrol) ──→ 2.4 (chase + scream) ──→ 2.9 (placement)
                                                       └──→ 2.6 (tile collision) ──┘                          │
...
|----------|---------|--------------|-------------|
| **Guard interaction model** | (A) Stomp-to-kill, (B) Pure avoidance, (C) Sanity-dependent | 2.1, 2.5, 3.1, 3.5 | **(C) Sanity-dependent** — ties directly into the essential experience. At Feral you're powerful enough to stomp; at Lucid you must dodge. Creates the power/control decision. But (B) is simpler to implement and still works. |
...
| **Guard guard assignment** | (A) Nearest civilian, (B) Manually assigned in level data | 2.2, 2.3 | **(A) Nearest same-platform civilian** — automatic at level load. Simpler, and placement is the level design lever. Manual assignment is unnecessary complexity for v1. |
